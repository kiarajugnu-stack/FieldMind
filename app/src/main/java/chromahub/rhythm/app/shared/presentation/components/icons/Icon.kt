...
private fun iconNameToImageVector(name: String, filled: Boolean): ImageVector {
    return iconMap[name] ?: when (name) {
        // Exact matches or close alternatives
        ...
        "arrow_upward", "expand_less", "up" -> Icons.Filled.ArrowBack // approximate
        "arrow_downward", "expand_more", "down" -> Icons.Filled.ArrowForward // approximate
        // Category icons
...
/**
 * Custom mapping for icon names not handled by the when-expression above.
 * This is populated with commonly used Material Symbol names and their closest ImageVector match.
 */
